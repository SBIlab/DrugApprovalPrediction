...
def rf(feature):
	...
	df_proba_final.to_csv('../../result/Results_in_paper/temp_file/df_proba_' + f_name + '.tsv', sep = '\t', header = True, index = False)

	### Save the file for prediction performance ###
	df_perf.to_csv('../../result/Results_in_paper/temp_file/df_performance_' + f_name + '.tsv', sep = '\t', header = True, index = False)
...
dir_list = os.listdir('../../result/Results_in_paper/')
if('temp_file' not in dir_list):
	os.system('mkdir ../../result/Results_in_paper/temp_file')
...
proba_file_list = [x for x in os.listdir('../../result/Results_in_paper/temp_file') if 'proba' in x]
perf_file_list = [x for x in os.listdir('../../result/Results_in_paper/temp_file') if 'performance' in x]

n = 0
for file_ in proba_file_list:
	if(n == 0):
		df_sum = pd.read_csv('../../result/Results_in_paper/temp_file/' + file_, sep = '\t')
		...
		df_temp = pd.read_csv('../../result/Results_in_paper/temp_file/' + file_, sep = '\t')
		df_sum = pd.merge(df_sum, df_temp, on = 'STITCH ID', how = 'left')
	os.system('rm -rf ../../result/Results_in_paper/temp_file/' + file_)
### Save the file that contains the approval probabilities for drugs ###
df_sum.to_csv('../../result/Results_in_paper/df_approval_probability.tsv', sep = '\t', header = True, index = False)
...
df_sum.to_csv('../../result/Results_in_paper/df_prediction_performance.tsv', sep = '\t', header = True, index = False)
os.system('rm -rf ../../result/Results_in_paper/temp_file')
