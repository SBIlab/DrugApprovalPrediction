...
def rf(feature):
	...
	df_proba_final.to_csv('../../result/Results_in_paper/df_proba_' + f_name + '.tsv', sep = '\t', header = True, index = False)

	### Save the file for prediction performance ###
	df_perf.to_csv('../../result/Results_in_paper/df_performance_' + f_name + '.tsv', sep = '\t', header = True, index = False)
...
proba_file_list = [x for x in os.listdir('../../result/Results_in_paper') if 'proba' in x]
perf_file_list = [x for x in os.listdir('../../result/Results_in_paper') if 'performance' in x]

n = 0
for file_ in proba_file_list:
	if(n == 0):
		df_sum = pd.read_csv('../../result/Results_in_paper/' + file_, sep = '\t')
		...
		df_temp = pd.read_csv('../../result/Results_in_paper/' + file_, sep = '\t')
...
for file_ in perf_file_list:
	...
	os.system('rm -rf ../../result/Results_in_paper/' + file_)
